...
query2 = """
    ...
    where $m/@id=$o/@movie_id and $o/year>2009 and $o/year<2020 and $o/type="BEST-PICTURE"
    order by $o/year
...
query3 = """
for $o in //oscar, $m in //movie
where $o/year = 1993 and $m/@id=$o/@movie_id
return <oscar_93>{
  $o/type,
  <movie>{$m/name/text()}</movie>,

    for $p in //person, $b in //oscar
    where $b/year = 1993 and $b/type=$o/type and $b/@person_id=$p/@id
